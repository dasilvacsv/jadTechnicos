...
interface DeliveryNotePDFProps {
  ...
  bcvRate?: number // PROP AGREGADO PARA RECIBIR LA TASA REAL
}
...
export function DeliveryNotePDF({ 
  ...
  // Parse concept data - use the current order's conceptoOrden
  const concept = parseConceptoOrden(order.conceptoOrden)

  // Calculate amounts - prioritize the order's budget amount, then fall back to others.
  const amount = order.presupuestoAmount && Number(order.presupuestoAmount) > 0
    ? Number(order.presupuestoAmount)
    : concept.amount && concept.amount > 0
    ? concept.amount
    : deliveryNote?.amount
    ? Number(deliveryNote.amount)
    : 0

  // Check if IVA should be included - use the order's includeIVA flag
  ...
  // Calculate total - use concept total if available and IVA is included, otherwise calculate it
  const totalAmount = shouldIncludeIVA && concept.totalAmount && concept.totalAmount > 0
    ? concept.totalAmount
    : amount + ivaAmount
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          ...
          {order.appliances?.map((appliance: any, index: number) => (
            <View key={`app-${index}`} style={styles.tableRow}>
              ...
              <Text style={styles.priceCell}>
                {amount ? formatCurrencyDisplaySync(Number(amount)) : '-'}
              </Text>
          ...
          <View style={styles.totalRow}>
            ...
            <Text style={styles.totalValue}>{formatCurrencyDisplaySync(amount)}</Text>
          </View>
          ...
          {shouldIncludeIVA && (
            <View style={styles.ivaRow}>
              ...
              <Text style={[styles.totalValue, { color: '#2563eb' }]}>
                {formatCurrencyDisplaySync(ivaAmount)}
              </Text>
            </View>
          )}
          ...
          <View style={styles.finalTotal}>
            ...
            <Text style={styles.finalTotalValue}>
              {formatCurrencyDisplaySync(totalAmount)}
            </Text>
          </View>
        </View>
