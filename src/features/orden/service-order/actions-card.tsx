...
export function ActionsCard({
  ...
  // Verificar si puede aplicar garantía (incluyendo garantías vencidas)
  const canApplyWarranty =
    order.status === "DELIVERED" &&
    ((order.garantiaEndDate && new Date(order.garantiaEndDate) > new Date()) || order.garantiaIlimitada)

  // Verificar si puede extender garantía vencida
  const canExtendExpiredWarranty =
    order.status === "DELIVERED" &&
    order.garantiaEndDate &&
    new Date(order.garantiaEndDate) <= new Date() &&
  ...
  // Helper function to render the Edit Budget button - SIEMPRE disponible si hay concepto
  const renderEditBudgetButton = () => (
    <Button
      className="w-full bg-blue-600 hover:bg-blue-700 text-white"
      onClick={onShowEditPresupuestoDialog}
      disabled={!order.conceptoOrden} // Solo deshabilitado si no hay concepto
    >
  ...
  return (
    <Card className="overflow-hidden border-l-4 border-l-teal-500 dark:border-l-teal-400">
      ...
      <CardContent className="space-y-3">
        ...
        {order.status === "ASSIGNED" && (
          <div className="space-y-3">
            ...
            >
              <div className="flex items-center">
                <Wrench className="mr-2 h-4 w-4" />
                <span>Cambiar Estado de Presupuesto</span>
              </div>
            </Button>
          </div>
        )}
        ...
        {/* Estado: DELIVERED - Editar Presupuesto siempre disponible */}
        {order.status === "DELIVERED" && (
          <div className="space-y-3">
            {renderEditBudgetButton()}
            {renderConceptoDisplay()}
        ...
        {/* Nueva opción: Extender Garantía Vencida */}
        {canExtendExpiredWarranty && (
          <div className="space-y-3">
            <div className="bg-amber-50 dark:bg-amber-950/30 p-3 rounded-md border border-amber-200 dark:border-amber-800">
              <p className="text-amber-800 dark:text-amber-300 flex items-center gap-2 text-sm">
                <Clock className="h-4 w-4" />
                <span>Garantía vencida el {formatDate(order.garantiaEndDate, "dd/MM/yyyy")}</span>
              </p>
            </div>
            ...
            >
              ...
              <span>Extender Garantía Vencida</span>
            </Button>
            ...
            >
              <Shield className="mr-2 h-4 w-4" />
              <span>Reportar Daño (Garantía Vencida)</span>
            </Button>
          </div>
        )}
