...
export default function DesktopView({
  ...
  resetFilters,
  initialOrders,
  isReadOnly = false, // <-- NUEVA PROP CON VALOR POR DEFECTO
}: DesktopViewProps) {
  ...
  // NUEVO EFECTO PARA MANEJAR isReadOnly
  useEffect(() => {
    // Ocultar o mostrar la columna de checkboxes de selección
    const selectColumn = table.getColumn('select');
    if (selectColumn) {
      selectColumn.toggleVisibility(!isReadOnly);
    }
    // Ocultar o mostrar la columna de acciones
    const actionsColumn = table.getColumn('actions');
    if (actionsColumn) {
      actionsColumn.toggleVisibility(!isReadOnly);
    }
  }, [isReadOnly, table]);
                              ...
                              <TableCell 
                                ...
                                onClick={() => {
                                  if (cell.column.id !== "actions" && cell.column.id !== "select") {
                                    window.open(`/tecnico/ordenes/${row.original.id}`, '_blank');
                                  }
                                }}
                              >
