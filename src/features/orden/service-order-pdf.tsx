...
import { formatCurrencyWithSymbol, getBCVRates, formatForeignCurrencyToVes } from "@/lib/exchangeRates";
...
const ServiceOrderPDF: React.FC<ServiceOrderPDFProps> = ({
  ...
  const [bcvConversion, setBcvConversion] = useState<{
    rate: number;
    converted: string;
  } | null>(null);
  ...
  // Calculate amounts
  const amount = order.presupuestoAmount && Number(order.presupuestoAmount) > 0
    ? Number(order.presupuestoAmount)
    : isDeliveryNote && deliveryNote?.amount
  ...
  // Load BCV conversion when amount or currency changes
  useEffect(() => {
    const loadBcvConversion = async () => {
      if (showBcvConversion && amount > 0) {
        try {
          const conversion = await formatForeignCurrencyToVes(amount, useEuro ? 'EUR' : 'USD');
          setBcvConversion({
            rate: conversion.rate,
            converted: conversion.formattedVES
          });
        } catch (error) {
          console.error('Error loading BCV conversion:', error);
          setBcvConversion(null);
        }
      } else {
        setBcvConversion(null);
      }
    ...
    loadBcvConversion();
  }, [amount, useEuro, showBcvConversion]);
  ...
  // Helper function to format currency with the selected symbol
  const formatCurrencyDisplay = (value: number) => {
    const original = formatCurrencyWithSymbol(value, useEuro);
    if (showBcvConversion && bcvConversion) {
      return `${original} (${bcvConversion.converted} Bs)`;
    }
    return original;
  };

  // Helper function for total calculations with BCV
  const formatTotalWithBCV = (value: number) => {
    const original = formatCurrencyWithSymbol(value, useEuro);
    if (showBcvConversion && bcvConversion) {
      const totalInBs = value * bcvConversion.rate;
      const formattedBs = new Intl.NumberFormat("es-VE", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }).format(totalInBs);
      return `${original} (${formattedBs} Bs)`;
    }
    return original;
  };

  // Lógica para truncar el texto de las observaciones si es muy largo
  const fullObservations = conceptHeader ? `${conceptHeader} - ${conceptText}` : "";
  // Ajustado para el nuevo tamaño del contenedor
  const MAX_CHARS_OBSERVATIONS = 220; 
  const truncatedObservations = fullObservations.length > MAX_CHARS_OBSERVATIONS
    ? `${fullObservations.substring(0, MAX_CHARS_OBSERVATIONS)}...`
    : fullObservations;

  return (
    // ===== CAMBIO: Padding reducido de p-4 a p-3 y tamaño de fuente de 11px a 10px =====
    <div className="container mx-auto p-3 max-w-[210mm] bg-white text-[10px]" style={{ minHeight: '297mm' }}>
      {isDeliveryNote ? (
        // Delivery Note Layout
        <>
          {/* ===== CAMBIO: Margen reducido de mb-6 a mb-4 ===== */}
          <div className="flex justify-between items-center mb-4">
            <div className="flex-1 flex items-center">
              {/* ===== CAMBIO: Ancho de imagen reducido de 150 a 130 ===== */}
              <Image
                src={logo}
                alt="Company Logo"
                width={130}
            ...
            <div className="text-right" style={{ width: "200px" }}>
              ...
              {useEuro && (
                <div className="flex items-center justify-end mt-2">
                  <span className="text-xs border border-blue-400 p-1 bg-blue-100 font-semibold text-blue-800">PRECIOS EN EUROS</span>
                </div>
              )}
              {showBcvConversion && bcvConversion && (
                <div className="flex items-center justify-end mt-2">
                  <span className="text-xs border border-green-400 p-1 bg-green-100 font-semibold text-green-800">
                    TASA BCV: 1 {useEuro ? 'EUR' : 'USD'} = {bcvConversion.rate.toFixed(2)} Bs
                  </span>
                </div>
              )}
            </div>
          ...
          {/* ===== CAMBIO: Margen reducido de mb-6 a mb-4 ===== */}
          <p className="text-base font-bold text-center mb-4">TELEFONOS: {order.company?.phones || "0212-7617671 / 0212-7635224 / 0412-0210824"}</p>
          {/* ===== CAMBIO: Margen reducido de mb-6 a mb-4 ===== */}
          <div className="border-b-2 border-gray-800 mb-4"></div>
          {/* ===== CAMBIO: Margen reducido de mb-8 a mb-4 ===== */}
          <div className="mb-4">
            {/* ===== CAMBIO: Margen reducido de mb-6 a mb-4 ===== */}
            <h2 className="text-xl font-bold text-center mb-4">
              {isPresupuesto ? "PRESUPUESTO" : "NOTA DE ENTREGA"} N° {formattedOrderCode}
            </h2>
            {/* ===== CAMBIO: Gap reducido de gap-8 a gap-4 ===== */}
            ...
            <table className="w-full border-collapse border border-gray-300">
              ...
              <tbody>
                {order.appliances?.map((appliance: any, index: number) => (
                  <tr key={index}>
                    ...
                    <td className="border border-gray-300 p-2 text-right">
                      {amount ? formatCurrencyDisplay(Number(amount)) : '-'}
                    </td>
                    <td className="border border-gray-300 p-2 text-right">
                      {amount ? formatCurrencyDisplay(Number(amount)) : '-'}
                    </td>
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 p-2 text-right">{formatCurrencyDisplay(amount)}</td>
                </tr>
                {shouldIncludeIVA && (
                  <tr>
                    ...
                    <td className="border border-gray-300 p-2 text-right bg-gray-50 text-blue-600">
                      {formatCurrencyDisplay(ivaAmount)} 
                    </td>
                  </tr>
                )}
                <tr>
                  ...
                  <td className="border border-gray-300 p-2 text-right font-bold bg-gray-100">
                    {formatTotalWithBCV(totalAmount)}
          ...
          {(deliveryNote?.diagnostics || order.diagnostics) && (
             // ===== CAMBIO: Margen reducido de mb-8 a mb-4 =====
            <div className="mb-4">
              <h3 className="font-bold mb-2">DIAGNÓSTICO TÉCNICO:</h3>
          ...
          {/* ===== CAMBIO: Márgenes y gap reducidos drásticamente ===== */}
          <div className="grid grid-cols-2 gap-8 mt-8 mb-6">
           ...
           {/* ===== CAMBIO: Margen reducido de mt-10 a mt-6 ===== */}
        ...
        <>
          ...
          <p className="text-base font-bold text-center">TELEFONOS: {order.company?.phones || "0212-7617671 / 0212-7635224 / 0412-0210824"}</p>
           {/* ===== CAMBIO: Margen reducido de mb-3 a mb-2 ===== */}
            ...
            <div className="grid grid-cols-2 border border-gray-300 border-t-0">
              ...
              <div className="p-1">
                <div className="grid grid-cols-3 gap-1">
                  ...
                  <p className="text-xs col-span-2 break-words">{order.client?.address || "N/A"}</p>
                </div>
              </div>
            </div>
          </div>
                      ...
                      <td className="p-1 text-left text-xs">
                        {order.presupuestoAmount ? (
                          <div className="space-y-1">
                            {shouldIncludeIVA ? (
                              <>
                                <div className="border-b pb-1">Subtotal: {formatCurrencyDisplay(Number(order.presupuestoAmount))}</div>
                                <div className="border-b pb-1 text-blue-600">IVA (16%): {formatCurrencyDisplay(Number(order.presupuestoAmount) * 0.16)}</div>
                                <div className="pt-1 font-bold">Total: {formatTotalWithBCV(Number(order.presupuestoAmount) * 1.16)}</div>
                              </>
                            ) : (
                              formatCurrencyDisplay(Number(order.presupuestoAmount))
                            )}
                          </div>
                        ) : (
                          ' '
              ...
              {(order.garantiaIlimitada || (order.garantiaStartDate && order.garantiaEndDate)) ? (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    {/* ===== CAMBIO: Altura mínima reducida de 120px a 90px y se agrega break-words ===== */}
                    <div className="border border-gray-200 p-1 bg-gray-50 text-xs min-h-[90px] break-words">
          ...
          {/* ===== CAMBIO: Margen superior reducido de mt-14 a mt-8 ===== */}
          ...
          {/* ===== CAMBIO: Margen superior reducido de mt-3 a mt-2 ===== */}
