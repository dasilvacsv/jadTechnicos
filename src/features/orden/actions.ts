...
import { eq, and, desc, ne, inArray, lte, gte, sql, ilike, or } from "drizzle-orm"
...
export async function updateServiceOrderAppliance(
  ...
  try {
    // Get current appliances for this service order
    const currentAppliances = await db.query.serviceOrderAppliances.findMany({
      where: eq(serviceOrderAppliances.serviceOrderId, serviceOrderId),
      orderBy: [serviceOrderAppliances.createdAt]
    });

    if (applianceIndex < 0 || applianceIndex >= currentAppliances.length) {
      return { success: false, error: "Índice de electrodoméstico inválido" };
    }

    const applianceToUpdate = currentAppliances[applianceIndex];
    ...
    await db
      ...
      .where(eq(serviceOrderAppliances.id, applianceToUpdate.id));

    revalidatePath(`/ordenes/${serviceOrderId}`);
    return { success: true };
  } catch (error) {
    console.error("Error updating service order appliance:", error);
    return {
      success: false,
      error: "Error al actualizar el electrodoméstico",
  ...
  try {
    const orders = await db.query.serviceOrders.findMany({
      columns: {
        ...
        presupuestoAmount: true,
        paymentStatus: true,
        garantiaIlimitada: true,
        garantiaStartDate: true,
        garantiaEndDate: true,
        conceptoOrden: true,
        razonGarantia: true,
        garantiaPrioridad: true,
        fechaAgendado: true,
        fechaCaptacion: true,
...
// =================================================================
// ✅ INICIO: NUEVA FUNCIÓN Y LÓGICA DE FILTRADO AVANZADO
// =================================================================

// Interfaz para los filtros que enviaremos desde el cliente
export interface OrderFilters {
  ...
  fechaAgendadoRange?: { from?: string; to?: string };
  // No necesitas fechaCaptacionRange si no se usa para el PDF
}

export async function getFilteredServiceOrders(filters: OrderFilters) {
  try {
    // 1. Crear alias para las tablas para evitar ambigüedad en los JOINS
    const so = alias(serviceOrders, "so");
    const c = alias(clients, "c");
    const u = alias(users, "u");
    const soa = alias(serviceOrderAppliances, "soa");
    const ca = alias(clientAppliances, "ca");
    const at = alias(applianceTypes, "at");
    const ta = alias(technicianAssignments, "ta");
    const t = alias(technicians, "t");

    // 2. Construir la consulta base con todos los JOINS necesarios para filtrar
    let query = db.selectDistinct({ id: so.id }).from(so)
      .leftJoin(c, eq(so.clientId, c.id))
      .leftJoin(u, eq(so.createdBy, u.id))
      .leftJoin(soa, eq(so.id, soa.serviceOrderId))
      .leftJoin(ca, eq(soa.clientApplianceId, ca.id))
      .leftJoin(at, eq(ca.applianceTypeId, at.id))
      .leftJoin(ta, eq(so.id, ta.serviceOrderId))
      .leftJoin(t, eq(ta.technicianId, t.id));

    const conditions = [];

    // 3. Añadir condiciones dinámicamente según los filtros recibidos

    // Filtro Global (búsqueda general)
    if (filters.globalFilter) {
      const search = `%${filters.globalFilter}%`;
      conditions.push(or(
        ilike(so.orderNumber, search),
        ilike(c.name, search),
        ilike(t.name, search)
      ));
    }

    // Filtros Facetados (por categoría)
    if (filters.status?.length) conditions.push(inArray(so.status, filters.status));
    if (filters.paymentStatus?.length) conditions.push(inArray(so.paymentStatus, filters.paymentStatus));
    if (filters.appliance?.length) conditions.push(inArray(at.name, filters.appliance));
    if (filters.createdBy?.length) {
      const lowerCaseCreators = filters.createdBy.map(creator => creator.toLowerCase());
      conditions.push(sql`lower(${u.fullName}) IN ${lowerCaseCreators}`);
    }
    if (filters.technician?.length) {
       conditions.push(and(inArray(t.name, filters.technician), eq(ta.isActive, true)));
    }
    
    // Filtros de Rango de Fecha
    if (filters.dateRange?.from && filters.dateRange?.to) {
        conditions.push(and(
            gte(so.receivedDate, new Date(filters.dateRange.from)),
            lte(so.receivedDate, new Date(filters.dateRange.to))
        ));
    }
    if (filters.fechaAgendadoRange?.from && filters.fechaAgendadoRange?.to) {
        conditions.push(and(
            gte(so.fechaAgendado, new Date(filters.fechaAgendadoRange.from)),
            lte(so.fechaAgendado, new Date(filters.fechaAgendadoRange.to))
        ));
    }

    // 4. Aplicar todas las condiciones a la consulta
    if (conditions.length > 0) {
      query.where(and(...conditions));
    }
    
    // 5. Ejecutar la consulta para obtener solo los IDs de las órdenes que cumplen los filtros
    const filteredOrderIdsResult = await query;
    const filteredOrderIds = filteredOrderIdsResult.map(res => res.id);

    // Si no hay resultados, devolver un arreglo vacío para evitar errores
    if (filteredOrderIds.length === 0) {
        return [];
    }
    
    // 6. Finalmente, obtener los datos completos de esas órdenes (con todas las relaciones)
    //    Esto es mucho más eficiente que hacer JOINS en una consulta con muchos datos.
    const orders = await db.query.serviceOrders.findMany({
        where: inArray(serviceOrders.id, filteredOrderIds),
        with: { // Usar la misma estructura de relaciones que tu getServiceOrders original
            client: {
                with: { sucursal: true }
            },
            appliances: {
    ...
    // En caso de error, devolver un arreglo vacío para no romper la aplicación
    return [];
  }
}

// OBTENER ÓRDENES SOLO PARA UN TÉCNICO ESPECÍFICO
export async function getServiceOrdersForTechnician(technicianId: string) {
  try {
    ...
    // Reutilizamos la misma lógica de getServiceOrders, pero filtrando por los IDs
    const orders = await db.query.serviceOrders.findMany({
      where: inArray(serviceOrders.id, orderIds), // <-- FILTRO CLAVE
      // ... incluye las mismas relaciones que en getServiceOrders
      with: {
        client: { with: { sucursal: true } },
        appliances: { with: { clientAppliance: { with: { brand: true, applianceType: true } } } },
        technicianAssignments: { with: { technician: true }, where: (assignments, { eq }) => eq(assignments.isActive, true) },
    ...
    console.error("Error fetching technician service orders:", error);
    return { success: false, error: "Error al obtener las órdenes de servicio" };
  }
}

// OBTENER UNA ORDEN ESPECÍFICA, VERIFICANDO QUE PERTENEZCA AL TÉCNICO
export async function getServiceOrderByIdForTechnician(id: string, technicianId: string) {
  try {
    // Primero, verificamos que el técnico esté asignado a esta orden
    const assignment = await db.query.technicianAssignments.findFirst({
      where: and(
    ...
    if (!assignment) {
      return { success: false, error: "Acceso denegado o la orden no existe." };
    }

    // Si la asignación existe, obtenemos los detalles completos de la orden
    const order = await getServiceOrderById(id); // Reutilizamos tu función existente
    return order;

  } catch (error) {
    console.error("Error fetching service order for technician:", error);
    return { success: false, error: "Error al obtener la orden de servicio" };
  }
}
