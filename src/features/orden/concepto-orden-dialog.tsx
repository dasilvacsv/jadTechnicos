...
// El único cambio está aquí, en el max de bodyText
const formSchema = z.object({
  ...
  }).max(2500, { // <-- CAMBIO APLICADO: de 1000 a 2500
    message: "La descripción no puede exceder los 2500 caracteres",
  }),
...
interface ConceptoOrdenDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  serviceOrderId: string
  presupuestoAmount: string | number | null | undefined
  currentStatus: string
  userId: string
  diagnostics?: string 
}

export function ConceptoOrdenDialog({
  open,
  onOpenChange,
  serviceOrderId,
  presupuestoAmount,
  ...
  userId,
  diagnostics,
}: ConceptoOrdenDialogProps) {
  const router = useRouter()
  const { toast } = useToast()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [showConfirmation, setShowConfirmation] = useState(false)
  const isFromAssigned = currentStatus === "ASSIGNED"
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      headerText: "",
      bodyText: "",
      presupuestoAmount: presupuestoAmount ? String(presupuestoAmount) : "",
      includeIVA: false,
  ...
  }, [open, form, presupuestoAmount, diagnostics])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "¡Éxito!",
          description: isFromAssigned 
            ? "Presupuesto registrado correctamente" 
            : "Concepto de orden establecido y facturado correctamente",
          variant: "default",
        })
        onOpenChange(false)
        router.refresh()
      ...
      console.error("Error setting concepto orden:", error)
      toast({
        title: "Error",
      ...
      <DialogContent className="sm:max-w-[90vw] w-full max-h-[90vh] overflow-y-auto mx-auto my-4">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl font-semibold">
            ...
            <span>
              {isFromAssigned ? "Registrar Presupuesto" : "Establecer Concepto de Orden"}
            </span>
          </DialogTitle>
        </DialogHeader>
            ...
            >
              <div className="bg-muted p-6 rounded-lg space-y-4">
                <h3 className="font-medium text-lg flex items-center gap-2">
                  ...
                  {isFromAssigned ? "Resumen del Presupuesto" : "Resumen del Concepto"}
                </h3>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-amber-800 dark:text-amber-300 font-medium">
                      {isFromAssigned ? "¿Confirmar Presupuesto?" : "¿Confirmar y Facturar?"}
                    </p>
                    <p className="text-amber-700/80 dark:text-amber-300/80 text-sm mt-1">
                      {isFromAssigned 
                        ? "Al confirmar, la orden pasará al estado \"Presupuestado\" y quedará lista para aprobación del cliente."
                        : "Al confirmar, la orden pasará al estado \"Facturado\" y se establecerá el concepto de orden."
                      }
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-base">
                          Título {isFromAssigned ? "del Presupuesto" : "del Concepto"} <span className="text-destructive">*</span>
                        </FormLabel>
                        <FormControl>
                          <Input
                            placeholder={isFromAssigned 
                              ? "Ej: Presupuesto Reparación de Lavadora Samsung" 
                              : "Ej: Reparación de Lavadora Samsung"
                            }
                            {...field}
                            className="h-10"
                          />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-base">
                          Descripción {isFromAssigned ? "del Presupuesto" : "del Concepto"} <span className="text-destructive">*</span>
                        </FormLabel>
                        <FormControl>
                          <Textarea
                            placeholder={isFromAssigned 
                              ? "Detalle los trabajos a realizar. Por ejemplo:\n\n- Diagnóstico completo del equipo\n- Reemplazo de motor principal\n- Cambio de rodamientos\n- Limpieza general" 
                              : "Detalle los trabajos realizados. Por ejemplo:\n\n- Diagnóstico completo del equipo\n- Reemplazo de motor principal\n- Cambio de rodamientos\n- Limpieza general\n- Pruebas de funcionamiento"
                            }
                            {...field}
                            rows={8}
                            className="resize-none"
                        ...
                        <FormDescription>
                          Detalle todos los trabajos {isFromAssigned ? "a realizar" : "realizados"}. Use guiones (-) para listar los items.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
