...
const formSchema = z.object({
  ...
  }).max(2500, { // AUMENTADO de 1000 a 2500 caracteres
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const conceptoOrden = {
        ...
        amount: parseFloat(values.presupuestoAmount).toString(),
        includeIVA: values.includeIVA,
      ...
      const updateData = {
        conceptoOrden,
        presupuestoAmount: parseFloat(values.presupuestoAmount).toString(),
        includeIVA: values.includeIVA,
...
Incluya todas las reparaciones, repuestos necesarios y cualquier trabajo adicional que se realizará en el electrodoméstico."
                            {...field}
                            rows={12} // Aumentado para permitir más texto
                            className="resize-none"
                          />
                        </FormControl>
                        <FormDescription>
                          <div className="flex flex-col gap-1">
                            <span>Detalle todos los trabajos a realizar. Use guiones (-) para listar los items.</span>
                            <span className="text-xs text-muted-foreground">
                              Límite: {field.value?.length || 0}/2500 caracteres
                            </span>
                          </div>
                        </FormDescription>
                        <FormMessage />
