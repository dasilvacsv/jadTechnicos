...
export function ServiceOrderDetail({ order, userId, isReadOnly = false }: ServiceOrderDetailProps) {
  // <-- NUEVA PROP
  ...
  const handleDeactivateTechnician = async (technicianId: string): Promise<void> => {
    // Bloquear acción si es solo lectura
    if (isReadOnly) {
      toast({ title: "Modo de solo lectura", description: "No se pueden realizar acciones.", variant: "destructive" })
      return
    }
    try {
  ...
  const handleToggleNotifications = async (enabled: boolean): Promise<void> => {
    if (isReadOnly) {
      toast({ title: "Modo de solo lectura", description: "No se pueden realizar acciones.", variant: "destructive" })
      setClientNotificationsEnabled(!enabled) // Revertir el cambio visual
      return Promise.reject(new Error("Read-only mode"))
    }
    try {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Éxito",
          description:
            values.cancellationType === "reschedule"
        ...
        if (currentOrder.garantiaPrioridad) {
          ...
          switch (currentOrder.garantiaPrioridad) {
            ...
            case "BAJA":
              ...
              break
            default:
              prioridadText = currentOrder.garantiaPrioridad
          }
    ...
    if (currentOrder.cancellationDate) {
      parts.push(
        <p className="text-red-600 dark:text-red-400 text-sm flex items-center gap-1 mb-3">
          ...
          <span>
            Cancelada el{" "}
            {new Date(currentOrder.cancellationDate).toLocaleDateString("es-ES", {
              year: "numeric",
              month: "long",
              day: "numeric",
              hour: "2-digit",
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            <div className="flex flex-col items-center space-y-4">
              ...
              <h2 className="text-3xl font-bold text-red-600 dark:text-red-400">Orden Cancelada</h2>

              <div className="w-full">{getCancellationReasonOrDates()}</div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              <Button className="w-full sm:w-1/2 bg-slate-600 hover:bg-slate-700" onClick={() => router.push("/ordenes")}>
              ...
              <Button
                ...
                disabled={isReadOnly} // <-- DESHABILITAR BOTÓN DE REACTIVAR
              >
      ...
      <HeaderSection order={currentOrder} onShowStatusDialog={handleShowStatusDialog} isReadOnly={isReadOnly} />

      {/* Aplicar opacidad y deshabilitar eventos si es cancelada o solo lectura */}
      <div className={isCancelled || isReadOnly ? "opacity-60 pointer-events-none" : ""}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <TechnicianCard
              ...
              isReadOnly={isReadOnly} // <-- Pasar prop
            />
            <OrdenStatusManager order={currentOrder} userId={userId} isReadOnly={isReadOnly} />
          </div>
