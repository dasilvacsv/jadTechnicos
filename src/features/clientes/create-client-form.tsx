...
export function ClientForm({ initialData, mode = "create", userId, onSuccess, onSubmit, closeDialog }: ClientFormProps) {
  ...
  const handleFormSubmit = useCallback(async (data: ClientFormData) => {
    ...
    try {
      ...
      closeDialog ? closeDialog() : setOpen(false)
      form.reset()
    }
  }, [isSubmitting, onSubmit, closeDialog, mode, userId, initialData?.id, toast, onSuccess, form, setOpen])
  ...
  return (
    <div className="sm:max-w-[425px] md:max-w-[600px] max-h-[80vh] overflow-hidden flex flex-col">
      ...
      <DialogHeader className="flex-shrink-0 px-6 py-4 border-b">
        ...
        <DialogDescription className="text-sm mt-1 flex items-center justify-between">
          <span>Complete los datos del cliente. Todos los campos marcados con * son obligatorios.</span>
          <div className="flex items-center gap-2">
            <label htmlFor="copy-mode" className="text-xs text-muted-foreground">
              Modo Manual
            </label>
            <input
              type="checkbox"
          ...
          <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6 py-6">
            ...
            <DialogFooter className="flex space-x-2 mt-1 px-6 py-4 border-t flex-shrink-0">
              <Button
                type="button"
                variant="outline"
                onClick={() => closeDialog ? closeDialog() : setOpen(false)}
